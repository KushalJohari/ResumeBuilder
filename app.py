...
for i, exp in enumerate(st.session_state.experience):
    with st.expander(f"Experience {i + 1}", expanded=True):
        ...
        st.session_state.experience[i]["Time"] = f"{st.selectbox('Start Month',months,key="start_month_{i}")} - {st.selectbox('End Month', months, key="end_month_{i}")}, {st.selectbox('Year', Years, key="year_{i}")}"
